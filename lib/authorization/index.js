...
function init (env, ctx) {
  ...
  function extractJWTfromRequest (req) {
    ...
    if (!token) {
      ...
      if (accessToken) {
        console.warn('[SECURITY] Token passed via query string from IP ' + getRemoteIP(req) + ' - use Authorization header instead. Query string tokens are logged by proxies and servers.');
        delete req.query.token; // remove from query to prevent re-logging
      }
  ...
  function apiSecretFromRequest (req) {
    ...
    let secret = req.header('api-secret');
    if (!secret && req.query && req.query.secret) {
      console.warn('[SECURITY] API secret passed via query string from IP ' + getRemoteIP(req) + ' - use api-secret header instead. Query string secrets are logged by proxies and servers.');
      secret = req.query.secret;
      delete req.query.secret; // remove from query to prevent re-logging
    }
  ...
  authorization.resolve = async function resolve (data, callback) {
    ...
    if (data.api_secret === 'null') { // some clients incorrectly send the string "null"
    ...
    } catch (err) { /* invalid JWT - token remains null */ }
