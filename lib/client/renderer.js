...
function init (client, d3) {
  ...
  renderer.addFocusCircles = function addFocusCircles () {
    ...
    function focusCircleTooltip (d) {
      ...
      client.tooltip.html('<strong>' + translate('BG') + ':</strong> ' + client.sbx.scaleEntry(d) +
          (d.type === 'mbg' ? '<br/><strong>' + translate('Device') + ': </strong>' + escapeHtml(d.device) : '') +
          (d.type === 'forecast' && d.forecastType ? '<br/><strong>' + translate('Forecast Type') + ': </strong>' + escapeHtml(d.forecastType) : '') +
          (rawbgInfo.value ? '<br/><strong>' + translate('Raw BG') + ':</strong> ' + escapeHtml(rawbgInfo.value) : '') +
          (rawbgInfo.noise ? '<br/><strong>' + translate('Noise') + ':</strong> ' + escapeHtml(rawbgInfo.noise) : '') +
  ...
  renderer.addTreatmentCircles = function addTreatmentCircles (nowDate) {
    ...
    function announcementTooltip (d) {
      return '<strong>' + translate('Time') + ':</strong> ' + client.formatTime(getOrAddDate(d)) + '<br/>' +
        ...
        (d.notes && d.notes.length > 1 ? '<strong>' + translate('Message') + ':</strong> ' + escapeHtml(d.notes) + '<br/>' : '') +
        (d.enteredBy ? '<strong>' + translate('Entered By') + ':</strong> ' + escapeHtml(d.enteredBy) + '<br/>' : '');
    }
  ...
  function appendTreatments (treatment, arc) {
    ...
    function treatmentTooltip () {
      ...
      client.tooltip.html('<strong>' + translate('Time') + ':</strong> ' + client.formatTime(getOrAddDate(treatment)) + '<br/>' + '<strong>' + translate('Treatment type') + ':</strong> ' + translate(client.careportal.resolveEventName(treatment.eventType)) + '<br/>' +
          ...
          (treatment.enteredinsulin ? '<strong>' + translate('Combo Bolus') + ':</strong> ' + escapeHtml(treatment.enteredinsulin) + 'U, ' + escapeHtml(treatment.splitNow) + '% : ' + escapeHtml(treatment.splitExt) + '%, ' + translate('Duration') + ': ' + escapeHtml(treatment.duration) + '<br/>' : '') +
          (treatment.glucose ? '<strong>' + translate('BG') + ':</strong> ' + glucose + (treatment.glucoseType ? ' (' + translate(treatment.glucoseType) + ')' : '') + '<br/>' : '') +
          (treatment.enteredBy ? '<strong>' + translate('Entered By') + ':</strong> ' + escapeHtml(treatment.enteredBy) + '<br/>' : '') +
          (treatment.notes ? '<strong>' + translate('Notes') + ':</strong> ' + escapeHtml(treatment.notes) : '') +
  ...
  renderer.addTreatmentProfiles = function addTreatmentProfiles (client) {
    ...
    function profileTooltip (d) {
      return '<strong>' + translate('Time') + ':</strong> ' + client.formatTime(getOrAddDate(d)) + '<br/>' +
        (d.eventType ? '<strong>' + translate('Treatment type') + ':</strong> ' + translate(client.careportal.resolveEventName(d.eventType)) + '<br/>' : '') +
        (d.endprofile ? '<strong>' + translate('End of profile') + ':</strong> ' + escapeHtml(d.endprofile) + '<br/>' : '') +
        (d.profile ? '<strong>' + translate('Profile') + ':</strong> ' + escapeHtml(d.profile) + '<br/>' : '') +
        (d.duration ? '<strong>' + translate('Duration') + ':</strong> ' + escapeHtml(d.duration) + translate('mins') + '<br/>' : '') +
        (d.enteredBy ? '<strong>' + translate('Entered By') + ':</strong> ' + escapeHtml(d.enteredBy) + '<br/>' : '') +
        (d.notes ? '<strong>' + translate('Notes') + ':</strong> ' + escapeHtml(d.notes) : '');
    }
