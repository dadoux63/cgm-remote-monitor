...
const init = function init () {
  ...
  const apiKeySHA256 = Symbol('api-secretSHA256');
  ...
  enclave.setApiKey = function setApiKey (keyValue) {
    ...
    secrets[apiKeySHA256] = genHash(keyValue,'sha256');
  ...
  enclave.isApiKey = function isApiKey (keyValue) {
    return keyValue === secrets[apiKeySHA256] || keyValue === secrets[apiKeySHA512];
  }
  ...
  enclave.getSubjectHash = function getSubjectHash(id) {
    ...
    shasum.update(secrets[apiKeySHA256]);
    shasum.update(id);
