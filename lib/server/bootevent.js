...
function boot (env, language) {
  ...
  //////////////////////////////////////////////////
  // Check Node version.
  // Node 18+ LTS is recommended and supported.
  ...
  function checkNodeVersion (ctx, next) {
    ...
    if (isLTS && (semver.satisfies(nodeVersion, '>=18.0.0'))) {
      console.debug('Node LTS version ' + nodeVersion + ' is supported');
      next();
      return;
    }

    // Also allow recent non-LTS versions in development/test environments
    if (process.env.NODE_ENV !== 'production' && semver.satisfies(nodeVersion, '>=18.0.0')) {
      console.warn('Node version ' + nodeVersion + ' is not an LTS release. For production use an LTS version.');
      next();
      return;
    }

    console.log( 'ERROR: Node version ' + nodeVersion + ' is not supported. Please use Node 18 LTS or newer.');
    process.exit(1);
