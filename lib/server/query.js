...
var SAFE_REGEX_FLAGS = /^[gimsuy]*$/;
var MAX_REGEX_LENGTH = 100;

function parseRegEx (str) {
  var regtest = /\/(.*)\/(.*)/.exec(str);
  if (regtest) {
    var pattern = regtest[1];
    var flags = regtest[2];

    if (pattern.length > MAX_REGEX_LENGTH) {
      console.error('Rejected regex pattern exceeding max length:', pattern.length);
      return str;
    }

    if (!SAFE_REGEX_FLAGS.test(flags)) {
      console.error('Rejected regex with invalid flags:', flags);
      return str;
    }
    ...
    try {
      ...
      console.error('Rejected invalid regex pattern:', e.message);
      return str;
    }
  }
  return str;
